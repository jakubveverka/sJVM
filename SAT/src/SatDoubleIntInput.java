...
public class SatDoubleIntInput {
    ...
    public static int solve(int[] formula, int combination) {
        int prevResult = -1;
    ...
    public static void main(String[] args) {
        ...
        for(int combination = 0; combination < countOfPossibleCombinations; combination++) {
            ...
            for(int i = 0; i < formula.length-1; i+=2) {
                ...
                if(formula[i+2][0] == -3 && formula[i+2].length == 1) {
                    moduloAdd = 1;
                }
                if(formula[i+2+moduloAdd].length > 1) {
                    nextResult = (solve(formula[i+2+moduloAdd], combination) + moduloAdd) % 2;
                } else {
